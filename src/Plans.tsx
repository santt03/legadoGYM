...
const emptyWeeks = (): Week[] => [1, 2, 3, 4].map((weekNumber) => ({ weekNumber, days: [] }))
const emptyItem = (exerciseId: string): Item => ({ exerciseId, sets: 3, reps: '10', load: '', restSeconds: 60, notes: '' })
const emptyDietWeeks = (): DietWeek[] => [1, 2, 3, 4].map((weekNumber) => ({ weekNumber, days: [] }))
const emptyDietItem = (foodId: string): DietItem => ({ foodId, quantity: '', grams: null, notes: '' })
...
function FoodOptions({ foods, groups }: { foods: Food[]; groups: FoodGroup[] }) {
  return <>{groups.map((group) => {
    const items = foods.filter((food) => food.groupId === group.id)
    return items.length ? <optgroup key={group.id} label={group.name}>{items.map((food) => <option key={food.id} value={food.id}>{food.name}</option>)}</optgroup> : null
  })}{foods.some((food) => !food.groupId) && <optgroup label="Sin grupo">{foods.filter((food) => !food.groupId).map((food) => <option key={food.id} value={food.id}>{food.name}</option>)}</optgroup>}</>
}
...
const dayGroups = (value: string) => muscleGroups.filter((group) => hasGroup(value, group.name))
  ...
  const addItem = (weekNumber: number, dayIndex: number) => {
    if (!exercises.length) { setError('Primero cargá ejercicios en el catálogo.'); return }
    const day = draft?.weeks.find((week) => week.weekNumber === weekNumber)?.days[dayIndex]
    if (day) {
      ...
      updateDay(weekNumber, dayIndex, { exercises: [...day.exercises, emptyItem(first.id)] })
    }
  }
  ...
  const savePlan = async () => {
    ...
    try {
      ...
      const saved = await request<Plan>(existing ? `/plans/${draft.id}` : '/plans', {
        ...
        body: JSON.stringify({ memberId: draft.memberId, name: draft.name, objective: draft.objective, startDate: draft.startDate, weeks: draft.weeks }),
      })
  ...
  const addDietDay = (weekNumber: number) => {
    if (!dietDraft) return
    updateDietWeeks(dietDraft.weeks.map((week) => week.weekNumber === weekNumber ? { ...week, days: [...week.days, { name: `Día ${week.days.length + 1}`, meals: [] }] } : week))
  }
  const removeDietDay = async (weekNumber: number, index: number) => {
    if (!dietDraft || !await confirm('¿Quitar este día?', 'Se quitarán del borrador el día y todas sus comidas.')) return
  ...
  const addDietItem = (weekNumber: number, dayIndex: number, mealIndex: number) => {
    if (!foods.length) { setError('Primero cargá alimentos en el catálogo.'); return }
    const meal = dietDraft?.weeks.find((w) => w.weekNumber === weekNumber)?.days[dayIndex]?.meals[mealIndex]
    if (meal) updateDietMeal(weekNumber, dayIndex, mealIndex, { items: [...meal.items, emptyDietItem(foods[0].id)] })
  }
  ...
  const saveDiet = async () => {
    ...
    try {
      ...
      const saved = await request<Diet>(existing ? `/diets/${dietDraft.id}` : '/diets', {
        method: existing ? 'PUT' : 'POST',
        body: JSON.stringify({ memberId: dietDraft.memberId, name: dietDraft.name, objective: dietDraft.objective, startDate: dietDraft.startDate, weeks: dietDraft.weeks }),
      })
